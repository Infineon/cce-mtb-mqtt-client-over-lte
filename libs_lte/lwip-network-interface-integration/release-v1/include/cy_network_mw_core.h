...
typedef enum
{
    ...
    CY_NETWORK_PPP_INTERFACE         = 2,  /**< PPP interface */
    CY_NETWORK_ETH_INTERFACE         = 3   /**< ethernet interface */
...
#if defined(CYBSP_PPP_CAPABLE)
/* Function pointer for PPP status callback */
typedef void (*cy_link_status_callback_t)( void *err_code );
#endif
...
#if defined(CYBSP_PPP_CAPABLE)
/* APIs for PPP connection */

/* Set the status callback function provided by user
 * @param[in] link_status_cb : status callback function pointer from user
 *
 * @return CY_RSLT_SUCCESS if successful
 * */
