...
extern cy_worker_thread_info_t       cy_cm_worker_thread;
...
static void rx_data_event_handler(void* arg)
{
    ...
    {
        cy_rtos_put_queue(&rx_input_buffer_queue, &rx_buffer_info, CY_RTOS_NEVER_TIMEOUT, 0);
        return;
    }

#if ETH_PAD_SIZE
    rx_buffer_info->length += ETH_PAD_SIZE;    /* Allow room for Ethernet padding */
#endif

    /* Allocate a pbuf chain of pbufs from the pool. */
    p = pbuf_alloc(PBUF_RAW, (u16_t)rx_buffer_info->length, PBUF_POOL);
    if (p != NULL)
    {
#if ETH_PAD_SIZE
        ...
        {
            MEMCPY(q->payload, &(rx_buffer_info->rx_data_ptr[len]), q->len);
            len += q->len;
        }
#if ETH_PAD_SIZE
        ...
        */
        ...
        result = cy_rtos_put_queue(&rx_input_buffer_queue, &rx_buffer_info, CY_RTOS_NEVER_TIMEOUT, 0);
...
void cy_process_ethernet_data_cb( ETH_Type *eth_type, uint8_t * rx_buffer, uint32_t length )
{
    ...
    /* Copy the Ethernet data from the pdl buffer to the middleware buffer*/
    memcpy((void *)(pbuffer + sizeof(cy_rx_buffer_info_t)), (void *)rx_buffer, length);

    if((cy_worker_thread_enqueue(&cy_cm_worker_thread, rx_data_event_handler, pbuffer)) != CY_RSLT_SUCCESS)
    {
...
static err_t ethif_output(struct netif *netif, struct pbuf *p)
{
    ...
    for(q = p; q != NULL; q = q->next)
    {
        MEMCPY((data_buffer + framelen), q->payload, q->len);
        framelen += (uint32_t)q->len;
    }
    ...
    pbuf_header(p, ETH_PAD_SIZE);    /* Reclaim the padding word. */
        ...
        if(if_ctx->iface_idx == (uint8_t)CY_ECM_INTERFACE_ETH1)
        {
            ...
            ethStatus = Cy_ETHIF_TransmitFrame(ETH1, (uint8_t*)data_buffer, framelen, CY_ETH_QS0_0, true);
        ...
        else
        {
            ...
            ethStatus = Cy_ETHIF_TransmitFrame(ETH0, (uint8_t*)data_buffer, framelen, CY_ETH_QS0_0, true);
            Cy_ETHIF_ClearReleasedBuf(true, true);
            if(ethStatus != CY_ETHIF_SUCCESS)
            {
                cm_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "failed to send outgoing packet :[%d]\n", ethStatus);
