...
#if defined(CYBSP_ETHERNET_CAPABLE)
#ifdef COMPONENT_CAT1
#define CM_WORKER_THREAD_PRIORITY                (CY_RTOS_PRIORITY_ABOVENORMAL)
#define CM_WORKER_THREAD_STACK_SIZE              (4 * 1024)
#endif
#endif

#if defined(CYBSP_PPP_CAPABLE)
#define CM_PPP_WORKER_THREAD_PRIORITY            (CY_RTOS_PRIORITY_ABOVENORMAL)
#define CM_PPP_WORKER_THREAD_STACK_SIZE          (2 * 1024)
#define CY_PPP_DATA_PROCESS_THREAD_STACK_SIZE    (4 * 1024)
#define CM_PPP_EVENT_TIMEOUT                     (0xffff)
#define CM_PPP_USER_INTERRUPT                    (1)
#define CM_PPP_BUFFER_SIZE                       (1536)
#endif
/******************************************************
...
#define SET_IP_NETWORK_INITED(interface_index, status)   (ip_networking_inited[(interface_index)&7] = status)

/* Interface UP status */
static bool ip_up[CY_IFACE_MAX_HANDLE];
#define SET_IP_UP(interface_index, status)               (ip_up[(interface_index)&7] = status)

struct  netif                                           *cy_lwip_ip_handle[CY_IFACE_MAX_HANDLE];
#define LWIP_IP_HANDLE(interface)                        (cy_lwip_ip_handle[(interface) & 7])

static struct netif       sta_ip_handle;
static struct netif       ap_ip_handle;
static struct netif       ppp_ip_handle;
...
#if defined(CYBSP_ETHERNET_CAPABLE)
#ifdef COMPONENT_CAT1
cy_worker_thread_info_t       cy_cm_worker_thread;
cy_queue_t                    rx_input_buffer_queue;
static cy_internal_buffer_pool_t *pool_handle = NULL;
#endif
#endif

#if defined(CYBSP_PPP_CAPABLE)
static cy_worker_thread_info_t       cm_ppp_status_thread;
cy_link_status_callback_t            ppp_link_status_cb;
cy_event_t                           cm_event;
ppp_pcb                              *ppp_pcb_handle = NULL;
static cy_thread_t                   ppp_data_thread = NULL;
#endif
...
#if defined(CYBSP_PPP_CAPABLE)
static uint32_t pppoutput(ppp_pcb *pcb, u8_t *data, u32_t len, void *ctx);
static void pppstatus_cb(ppp_pcb *pcb, int err_code, void *ctx);
static void cy_network_process_ppp_data(cy_thread_arg_t arg);
#endif
/******************************************************
...
#if defined(CYBSP_ETHERNET_CAPABLE)
#ifdef COMPONENT_CAT1
static cy_rslt_t cy_buffer_pool_create(uint32_t no_of_buffers, uint32_t sizeof_buffer)
{
    uint16_t header1;
    ...
    pool_handle = calloc(1, sizeof(cy_internal_buffer_pool_t));
    if (NULL == pool_handle)
    {
    ...
    header1 = sizeof(list_node_t);

    /* Allocate memory for the actual buffer */
    data_buffer = calloc( 1, (no_of_buffers * sizeof_buffer) + (header1 * no_of_buffers) );
    if (NULL == data_buffer)
    {
        cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG, "Failed to allocate memory for actual data\n");
        return CY_RSLT_NETWORK_ERROR_NOMEM;
    }
    cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG, "Base address of allocated buffer : [%0x]\n", data_buffer);
    ...
    while (count <= no_of_buffers)
    {
        ...
        node->next = NULL;
        node->buffer_ptr = data_buffer + header1;
        ...
        data_buffer += sizeof_buffer + header1;
...
static cy_rslt_t cy_buffer_pool_get(cy_buffer_t *buffer)
{
...
static void cy_buffer_pool_delete(void)
{
...
cy_rslt_t cy_network_init( void )
{
    ...
    params.name = "CM- Worker";
    params.priority = CM_WORKER_THREAD_PRIORITY;
    params.stack = NULL;
    params.stack_size = CM_WORKER_THREAD_STACK_SIZE;
    params.num_entries = 0;

    /* Create a worker thread */
    if(cy_worker_thread_create(&cy_cm_worker_thread, &params) != CY_RSLT_SUCCESS)
    {
    ...
    if(cy_buffer_pool_create(NO_OF_BUFFERS, (sizeof(cy_rx_buffer_info_t) + CY_ETH_SIZE_MAX_FRAME))!= CY_RSLT_SUCCESS)
    {
        cm_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "Failed to create buffer pool\n");
        return CY_RSLT_NETWORK_ERROR_NOMEM;
    }

    cm_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "Get all the buffers from pool and push to queue\n");

    for(int i = 0; i < pool_handle->total_num_buf_created; i++)
    {
        /* Get the buffer from pool */
        if(cy_buffer_pool_get(&get_buffer)!= CY_RSLT_SUCCESS)
        {
    ...
    /* Initialize RTOS event to signal PPP event */
    cy_rtos_init_event(&cm_event);

    /* create a worker thread for handling the PPP status callback */
    memset(&params, 0, sizeof(params));
    params.name = "CM_ppp_status_worker_thread";
    params.priority = CM_PPP_WORKER_THREAD_PRIORITY;
    params.stack = NULL;
    params.stack_size = CM_PPP_WORKER_THREAD_STACK_SIZE;
    params.num_entries = 0;
    if(cy_worker_thread_create(&cm_ppp_status_thread, &params) != CY_RSLT_SUCCESS)
    {
...
cy_rslt_t cy_network_deinit( void )
{
    ...
    else
    {
        ...
        if(connectivity_lib_init == 0)
        {
            ...
            cy_worker_thread_delete(&cy_cm_worker_thread);

            /* Delete the queue */
            cy_rtos_deinit_queue(&rx_input_buffer_queue);

            /* Delete the pool */
            cy_buffer_pool_delete();
#endif
#endif

#if defined(CYBSP_PPP_CAPABLE)
            /* Delete the  worker thread used for handling the PPP status callback */
            cy_worker_thread_delete(&cm_ppp_status_thread);

            /* De-initialize RTOS events */
            cy_rtos_deinit_event(&cm_event);
#endif
        }
    }
...
#if defined(CYBSP_PPP_CAPABLE)
    if( CY_NETWORK_PPP_INTERFACE == iface_context_database[index].iface_type )
    {
        /* Create the PPP handle */
        ppp_pcb_handle = pppapi_pppos_create(LWIP_IP_HANDLE(iface_context_database[index].iface_type), pppoutput, pppstatus_cb, NULL);
        if(ppp_pcb_handle == NULL)
        {
            cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG,"Failed to create PPP handle\n");
            return CY_RSLT_NETWORK_PPP_ERR_GENERIC;
        }
        /* Assign network instance to the iface database */
        iface_context_database[index].nw_interface = LWIP_IP_HANDLE(iface_context_database[index].iface_type);
        cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG,"iface_context_database[index].nw_interface:[%p]\n", iface_context_database[index].nw_interface);
...
void* cy_network_get_nw_interface(cy_network_hw_interface_type_t iface_type, uint8_t iface_idx)
{
    ...
    if( ((iface_type != CY_NETWORK_WIFI_STA_INTERFACE) && (iface_type != CY_NETWORK_WIFI_AP_INTERFACE)
            && (iface_type != CY_NETWORK_ETH_INTERFACE) && (iface_type != CY_NETWORK_PPP_INTERFACE))
...
#if LWIP_IPV4
#if defined(CYBSP_ETHERNET_CAPABLE) || defined(CYBSP_WIFI_CAPABLE)
...
#if defined(CYBSP_PPP_CAPABLE)
    /* Create the thread to read from uart */
    if((res = cy_rtos_create_thread(&ppp_data_thread, cy_network_process_ppp_data,
                                    "ppp_data_processing_thread",
                                     NULL, CY_PPP_DATA_PROCESS_THREAD_STACK_SIZE,
                                     CY_RTOS_PRIORITY_NORMAL, NULL)) != CY_RSLT_SUCCESS)
    {
        cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG,"task creation failed\n");
        return res;
    }

    /* Set PPP interface as the default interface */
    res = pppapi_set_default(ppp_pcb_handle);
    if(res != ERR_OK)
    {
        cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s(): Failed to set the default interface \n", __FUNCTION__ );
        return res;
    }

    /* Request for DNS server addresses from the peer */
    ppp_set_usepeerdns(ppp_pcb_handle, 1);

    /* Set the authentication type */
    ppp_set_auth(ppp_pcb_handle, PPPAUTHTYPE_ANY, "login", "password");

    res = pppapi_connect(ppp_pcb_handle, 0);
    if(res != ERR_OK)
    {
        cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s(): pppapi_connect failed \n", __FUNCTION__ );
        return res;
    }
#endif
...
cy_rslt_t cy_network_ip_down(cy_network_interface_context *iface)
{
    ...
    */
    ...
    res = pppapi_close(ppp_pcb_handle, 0);
    if(res != ERR_OK)
    {
        cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s(): pppapi_close failed \n", __FUNCTION__ );
        return res;
    }

    /* Wait for user interrupt in PPP status callback to confirm that PPP connection is closed */
    bits = CM_PPP_USER_INTERRUPT;
    cy_rtos_waitbits_event(&cm_event, &bits, true, false, CM_PPP_EVENT_TIMEOUT);

    /* free the PPP handle */
    res = pppapi_free(ppp_pcb_handle);
    if(res != ERR_OK)
    {
        cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s(): pppapi_free failed \n", __FUNCTION__ );
        return res;
    }

    if(ppp_data_thread != NULL)
    {
        cy_rtos_terminate_thread(&ppp_data_thread);
    }
#endif
...
static bool is_interface_added(uint8_t interface_index)
{
    return (ip_networking_inited[interface_index & 7]);
}

static bool is_network_up(uint8_t interface_index)
{
    return (ip_up[interface_index & 7]);
}
...
/* Helper function to convert the lwip PPP error code to connectivity middleware error code */
static cy_rslt_t lwip_ppp_to_nw_err_code(uint32_t err_code)
{
    switch(err_code) {
    case PPPERR_NONE:               /* No error. */
         return CY_RSLT_NETWORK_PPP_ERR_NONE;

    case PPPERR_PARAM:             /* Invalid parameter. */
         return CY_RSLT_NETWORK_PPP_ERR_PARAM;

    case PPPERR_OPEN:              /* Unable to open PPP session. */
        return CY_RSLT_NETWORK_PPP_ERR_OPEN;

    case PPPERR_DEVICE:            /* Invalid I/O device for PPP. */
        return CY_RSLT_NETWORK_PPP_ERR_DEVICE;

    case PPPERR_ALLOC:             /* Unable to allocate resources. */
        return CY_RSLT_NETWORK_PPP_ERR_ALLOC;

    case PPPERR_USER:              /* User interrupt. */
        return CY_RSLT_NETWORK_PPP_ERR_USER;

    case PPPERR_CONNECT:           /* Connection lost. */
        return CY_RSLT_NETWORK_PPP_ERR_CONNECT;

    case PPPERR_AUTHFAIL:          /* Failed authentication challenge. */
        return CY_RSLT_NETWORK_PPP_ERR_AUTHFAIL;

    case PPPERR_PROTOCOL:          /* Failed to meet protocol. */
        return CY_RSLT_NETWORK_PPP_ERR_PROTOCOL;

    case PPPERR_PEERDEAD:          /* Connection timeout. */
        return CY_RSLT_NETWORK_PPP_ERR_PEERDEAD;

    case PPPERR_IDLETIMEOUT:       /* Idle Timeout. */
        return CY_RSLT_NETWORK_PPP_ERR_IDLETIMEOUT;

    case PPPERR_CONNECTTIME:       /* Max connect time reached. */
        return CY_RSLT_NETWORK_PPP_ERR_CONNECTTIME;

    case PPPERR_LOOPBACK:          /* Loopback detected. */
        return CY_RSLT_NETWORK_PPP_ERR_LOOPBACK;
...
/* PPP data processing thread */
static void cy_network_process_ppp_data(cy_thread_arg_t arg)
{
    LWIP_UNUSED_ARG(arg);
    size_t size;
    uint32_t rx_index = 0, len=0;
    uint8_t buff[CM_PPP_BUFFER_SIZE];
    cyhal_uart_t *uart_obj = NULL;

    /* Get the hardware context for PPP */
    for (int i = 0; i < CY_IFACE_MAX_HANDLE; i++)
    {
        if( ( iface_context_database[i].is_initialized == true ) &&
            ( iface_context_database[i].iface_type == CY_NETWORK_PPP_INTERFACE ) )
        {
            uart_obj = (cyhal_uart_t *)(iface_context_database[i].hw_interface);
            break;
        }
    }
    if(uart_obj == NULL)
    {
        cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG,"Failed to get the Hardware interface for PPP\n");
        return;
    }
    while(1)
    {
        if((len = cyhal_uart_readable(uart_obj)) != 0 )
        {
            cyhal_syspm_lock_deepsleep();
            memset((void *)&buff[0] , 0 , sizeof(buff));
            rx_index = 0;
            do
            {
                size = (size_t)len;
                cyhal_uart_read(uart_obj, &buff[rx_index], &size);
                rx_index += len;
                cy_rtos_delay_milliseconds(10);
            } while(((len = cyhal_uart_readable(uart_obj)) != 0) && (rx_index < (sizeof(buff ) - len)));

            pppos_input_tcpip(ppp_pcb_handle, &buff[0], sizeof(buff));
...
/* Output callback function called from the lwip.
 * Passes the data to be written into uart to the user provided output callback function.
 */
static uint32_t pppoutput(ppp_pcb *pcb, u8_t *data, u32_t len, void *ctx)
{
    size_t size;
    uint32_t avail = 0, sent_index = 0;
    cyhal_uart_t *uart_obj;

    cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG," Amount of data to be written to uart: %ld\n", len);
    ...
    while(len != 0)
    {
        if((avail = cyhal_uart_writable(uart_obj))!=0)
        {
            if(len > avail)
            {
                size = avail;
            ...
            cyhal_uart_write(uart_obj, &data[sent_index], &size);
            len = len - size;
            sent_index = sent_index + size;
        }
    }
    cm_cy_log_msg( CYLF_MIDDLEWARE, CY_LOG_DEBUG,"Amount of data successfully written to uart: %ld\n", sent_index);
    return sent_index;
}

/* Status callback function called from the lwip.
 * Converts the lwip error code to lwip-network-interface-integration code
 * and send it to to the users status callback function.
 */
static void pppstatus_cb(ppp_pcb *pcb, int err_code, void *ctx)
{
    LWIP_UNUSED_ARG(ctx);
    ...
    cm_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "%d : %s Status_cb \r \n",__LINE__, __FUNCTION__);

    if(err_code == PPPERR_USER)
    {
        bits = CM_PPP_USER_INTERRUPT;
        cy_rtos_setbits_event(&cm_event, bits, false);
    }
    err = lwip_ppp_to_nw_err_code(err_code);

    if((res = cy_worker_thread_enqueue(&cm_ppp_status_thread, ppp_link_status_cb, &err)) != CY_RSLT_SUCCESS)
    {
        cm_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "L%d : %s() : ERROR : Failed to send status. Err = [%lu]\r\n", __LINE__, __FUNCTION__, res);
    }
}

cy_rslt_t cy_network_set_status_cb(cy_link_status_callback_t link_status_cb)
{
    if(link_status_cb == NULL)
    {
        return CY_RSLT_NETWORK_PPP_ERR_PARAM;
    }
    /* Set the users status callback function */
    ppp_link_status_cb = link_status_cb;
    return CY_RSLT_SUCCESS;
}
#endif
