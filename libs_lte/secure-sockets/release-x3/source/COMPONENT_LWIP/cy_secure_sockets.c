...
static cy_socket_ctx_t* alloc_socket()
{
    ...
    for(i = 0; i < NUM_SOCKETS; ++i)
    {
        ...
        if(!socket_list[i].ctx)
        {
            ...
            socket_list[i].ctx->iface_type = CY_SOCKET_ETH_INTERFACE;
#elif defined(CYBSP_PPP_CAPABLE)
            socket_list[i].ctx->iface_type = CY_SOCKET_PPP_INTERFACE;
#else
...
static cy_rslt_t eth_arp_resolve(ip4_addr_t *dest_addr, cy_socket_interface_t iface_type)
{
    ...
    if(iface_type != CY_SOCKET_STA_INTERFACE  && iface_type != CY_SOCKET_AP_INTERFACE && iface_type != CY_SOCKET_ETH_INTERFACE && iface_type != CY_SOCKET_PPP_INTERFACE)
    {
    ...
    else if(iface_type == CY_SOCKET_PPP_INTERFACE)
    {
        netif = cy_network_get_nw_interface(CY_NETWORK_PPP_INTERFACE, 0);
    }
    else
    {
        ...
        case CY_SOCKET_SOL_SOCKET:
            switch(optname)
            {
                ...
                case CY_SOCKET_SO_BINDTODEVICE:
                {
                    ...
                    else if(iface_type == CY_SOCKET_ETH_INTERFACE)
                    {
                        netif = cy_network_get_nw_interface(CY_NETWORK_ETH_INTERFACE, 0);
                    }
                    else if(iface_type == CY_SOCKET_PPP_INTERFACE)
