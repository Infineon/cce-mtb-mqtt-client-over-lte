...
static void cy_process_receive_event_with_valid_context(cy_socket_ctx_t *ctx)
{
             ...
             ( ( ctx->status & SOCKET_STATUS_FLAG_SECURED) ==  SOCKET_STATUS_FLAG_SECURED) )
    {
        ...
        if (ctx->callbacks.receive.callback)
        {
            nx_tcp_socket_bytes_available(ctx->nxd_socket.tcp, &bytes_available);
...
static cy_rslt_t network_send(void *context, const unsigned char *data_buffer, uint32_t data_buffer_length, uint32_t *bytes_sent)
{
    ...
    *bytes_sent = 0;
    ...
    if (ctx->transport_protocol == CY_SOCKET_IPPROTO_UDP)
    {
        ss_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "network_send called with UDP socket\n");
    ...
    while (data_buffer_length - *bytes_sent > 0)
    {
        ...
        ret = nx_tcp_socket_send(ctx->nxd_socket.tcp, packet, ctx->nonblocking ? TX_NO_WAIT : NX_TIMEOUT(ctx->send_timeout));
        if (ret != NX_SUCCESS)
        {
            /*
...
cy_rslt_t cy_socket_create(int domain, int type, int protocol, cy_socket_t *handle)
{
    ...
    if (protocol == CY_SOCKET_IPPROTO_TCP || protocol == CY_SOCKET_IPPROTO_TLS)
    {
        ...
        status = nx_tcp_socket_create(netif, ctx->nxd_socket.tcp, (CHAR*)"", NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, DEFAULT_TCP_WINDOW_SIZE, NX_NULL, cy_tcp_disconnect_callback);
    ...
    {
        status = nx_udp_socket_create(netif, ctx->nxd_socket.udp, (char*)"", 0, NX_DONT_FRAGMENT, 255, DEFAULT_UDP_QUEUE_SIZE);
        if (status == NX_SUCCESS)
        {
...
cy_rslt_t cy_socket_sendto(cy_socket_t handle, const void *buffer, uint32_t length, int flags, const cy_socket_sockaddr_t *dest_addr, uint32_t address_length, uint32_t *bytes_sent)
{
    ...
    *bytes_sent = 0;
    ...
    if (length > WHD_PAYLOAD_MTU)
    {
        ss_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "Length greater than MTU\n");
        return CY_RSLT_MODULE_SECURE_SOCKETS_BADARG;
    }
    ...
    memcpy(packet->nx_packet_prepend_ptr, buffer, length);
...
cy_rslt_t cy_socket_recvfrom(cy_socket_t handle, void *buffer, uint32_t length, int flags, cy_socket_sockaddr_t *src_addr, uint32_t *src_addr_length, uint32_t *bytes_received)
{
    ...
    *bytes_received = 0;
    ...
    do
    {
        err = nx_udp_socket_receive(ctx->nxd_socket.udp, &packet, ctx->nonblocking ? TX_NO_WAIT : NX_TIMEOUT(ctx->recv_timeout));
        if (err != NX_SUCCESS)
        {
    ...
    *bytes_received = (uint32_t)(packet->nx_packet_append_ptr - packet->nx_packet_prepend_ptr);
...
cy_rslt_t cy_socket_poll(cy_socket_t handle, uint32_t *rwflags, uint32_t timeout)
{
    ...
    if (*rwflags & CY_SOCKET_POLL_WRITE)
    {
        ss_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "Polling on write not supported\n");
        return CY_RSLT_MODULE_SECURE_SOCKETS_NOT_SUPPORTED;
    }
...
cy_rslt_t cy_socket_accept(cy_socket_t handle, cy_socket_sockaddr_t *address, uint32_t *address_length, cy_socket_t *socket)
{
    ...
    *socket = CY_SOCKET_INVALID_HANDLE;
    ...
    result = nx_tcp_socket_create(ctx->nxd_socket.tcp->nx_tcp_socket_ip_ptr, accept_ctx->nxd_socket.tcp, (CHAR*)"",
                                  NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, DEFAULT_TCP_WINDOW_SIZE, NX_NULL, cy_tcp_disconnect_callback);
    if (result != NX_SUCCESS)
    {
    ...
    /*
     * Don't copy the existing callbacks. Typically the callers have added a callback
     * argument to identify the socket for the callback event. If we copy the callback
     * info to the new socket, the callback argument will identify the listening
     * socket rather than the new socket. If the user wants a callback on the new
     * socket, they'll need to explicitly enable it.
     */
    /*
    ...
    *socket = accept_ctx;
    ...
    result = nxd_tcp_socket_peer_info_get(accept_ctx->nxd_socket.tcp, &peer_addr, &port);
    if (result != NX_SUCCESS)
    {
    ...
    {
        ss_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "Failed conversion from NetXDuo to secure socket\n");
        nx_tcp_server_socket_unaccept(accept_ctx->nxd_socket.tcp);
        nx_tcp_client_socket_unbind(accept_ctx->nxd_socket.tcp);
        nx_tcp_socket_delete(accept_ctx->nxd_socket.tcp);
        free_socket(accept_ctx);
        *socket = CY_SOCKET_INVALID_HANDLE;
    ...
    *address_length = sizeof(*address);
    ...
    if (ctx->enforce_tls)
    {
        tls_params.context = &accept_ctx->nxd_socket.tcp;
        tls_params.network_send = tls_network_send_callback;
        tls_params.network_recv = tls_network_receive_callback;
        tls_params.tls_identity = ctx->tls_identity;
        tls_params.auth_mode = ctx->auth_mode;
        ...
        ret = cy_tls_connect(accept_ctx->tls_ctx, CY_TLS_ENDPOINT_SERVER, ctx->send_timeout);
        if (ret != CY_RSLT_SUCCESS)
        {
            ss_cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "cy_tls_connect failed with error %lu\n", ret);
