...
static INT cy_tls_parse_private_key(const char *private_key, uint32_t private_key_len, UINT *private_key_type)
{
    unsigned char *header=NULL;
    unsigned char *footer=NULL;

    header = (unsigned char *) strnstr( private_key, "-----BEGIN RSA PRIVATE KEY-----", sizeof("-----BEGIN RSA PRIVATE KEY-----") );
    footer = (unsigned char *) strnstr( private_key, "-----END RSA PRIVATE KEY-----", private_key_len);
...
static INT cy_tls_convert_pem_to_der(const unsigned char *base64input, UINT inputlen, unsigned char *output, UINT *outputlen)
{
    const unsigned char *str1;
    const unsigned char *str2;
    const unsigned char *end = base64input + inputlen;
    int error;

    str1 = ( unsigned char * ) strstr( ( const char * ) base64input, "-----BEGIN" );
    if(str1 == NULL)
    {
        return( -1 );
    }

    str2 = (unsigned char *) strstr(( const char * ) base64input, "-----END");
    if(str2 == NULL)
    {
        return( -1 );
    }
    ...
    if(( str2 <= str1 ) || ( str2 > end ))
    {
        return(-1);
    }

    /* Decode the PEM certificate to DER */
    error = cy_tls_base64_decode((const unsigned char *) str1, str2 - str1, output, *outputlen, outputlen);
    if(error !=0)
    {
        return (-1);
    }
...
static cy_rslt_t cy_tls_internal_load_root_ca_certificates(NX_SECURE_X509_CERT** root_ca_certs, const char* trusted_ca_certificates, const uint32_t cert_length, unsigned char **root_ca_cert_der)
{
    ...
    *root_ca_cert_der = calloc(cert_length, 1);
    ...
    error = cy_tls_convert_pem_to_der((const unsigned char *)trusted_ca_certificates, cert_length, *root_ca_cert_der, &der_cert_len);
    if(error != 0)
    {
        free(*root_ca_certs);
        *root_ca_certs = NULL;
...
cy_rslt_t cy_tls_create_identity(const char *certificate_data, const uint32_t certificate_len, const char *private_key, uint32_t private_key_len, void **tls_identity)
{
    ...
    error = cy_tls_parse_private_key(private_key, private_key_len, &private_key_type);
    if ( error == -1 )
    {
    ...
    error = cy_tls_convert_pem_to_der((const unsigned char *)certificate_data, certificate_len, identity->certificate_der, &der_cert_len);
    if(error != 0)
    {
        free(identity->certificate_der);
    ...
    error = cy_tls_convert_pem_to_der((const unsigned char *)private_key, private_key_len, identity->private_key_der, &der_key_len);
    if(error != 0)
    {
        free(identity->certificate_der);
        free(identity->private_key_der);
...
cy_rslt_t cy_tls_connect(void *context, cy_tls_endpoint_type_t endpoint, uint32_t timeout)
{
...
#ifdef NX_SECURE_ENABLE_ECC_CIPHERSUITE
    error = nx_secure_tls_metadata_size_calculate(&nx_crypto_tls_ciphers_ecc, &metadata_size);
#else
    error = nx_secure_tls_metadata_size_calculate(&nx_crypto_tls_ciphers, &metadata_size);
#endif
    if (error != NX_SUCCESS)
    {
    ...
    /* Create TLS session */
#ifdef NX_SECURE_ENABLE_ECC_CIPHERSUITE
    error = nx_secure_tls_session_create(&ctx->tls_session, &nx_crypto_tls_ciphers_ecc, ctx->tls_metadata, metadata_size);
    if (error == NX_SUCCESS)
    {
...
#else
    error = nx_secure_tls_session_create(&ctx->tls_session, &nx_crypto_tls_ciphers, ctx->tls_metadata, metadata_size);
#endif
...
cy_rslt_t cy_tls_send(void *context, const unsigned char *data, uint32_t length, uint32_t timeout, uint32_t *bytes_sent)
{
    ...
    *bytes_sent = 0;
    ...
    error = nx_packet_data_append(packet, (void *)data, length, pool, NX_WAIT_FOREVER);
    if (error != NX_SUCCESS)
    {
